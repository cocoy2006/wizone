function createPageNav(opt) {
    ...
    function _initPageNav(){
        ...
        if(hasInput)   
            initStr.push('<div class="input-page-div">当前第<input type="text" maxlength="6" value="' + currentNum + '" />页，共<span>'
                + pageCount
                + '</span>页<button type="button" id="page-num" class="btn btn-xs input-btn-xs">确定</button></div>');
        initStr.push('</ul></nav>');
    ...
    function _createCommonPage(startNum, activeNum) {
        ...
        for (var i = 1,pageNum=startNum; i <= pageCount && i <= maxCommonLen; i++ , pageNum++) {
	            initStr.push('<li class="commonPage" value="' + pageNum + '"style="width:80px;height:80px;background:url(wibupt/img/building'+ i +'.jpg)"><p class="buildingName">'  +buildingName[i - 1]  +'</p></li>');
        }
